...
dncnn = dncnnModel.DnCNN()

dncnn.compile(optimizer=tf.keras.optimizers.Adam(1e-03), loss= ['mse'])
...
# TRAINING
noise_level = 50
dncnn.fit( train_dataset, shuffle=True,epochs=30, validation_data = test_dataset)
dncnn.save('dncnn.h5')
"""

## EVALUATION
noise_level = 50
dncnn = tf.keras.models.load_model(f"result_dcnn/noise_{noise_level}/dncnn.h5")

psrn_list = []
for i in range(200, 450):
  patches_noisy,denoised_patches,ground_truth,noisy_image,denoised_image=predict_fun(dncnn,f'data/test/{i}.png',noise_level=noise_level)
  print(i)
  ...
  path = f"result_dcnn/noise_{noise_level}/{i}/"
  os.mkdir(path)
  ...
  plot_predictions(ground_truth, noisy_image, denoised_image, f"result_dcnn/noise_{noise_level}/{i}.png")
