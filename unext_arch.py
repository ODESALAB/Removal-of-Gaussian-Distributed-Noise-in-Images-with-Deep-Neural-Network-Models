...
model = convnext.build_unet((40,40,1), n_classes=1)
model.compile(optimizer=tf.keras.optimizers.Adam(1e-03), loss= ['mse'])
model.summary()
...
# TRAINING
noise_level = 50
model.fit( train_dataset, shuffle=True, epochs=30, validation_data = test_dataset)
model.save('convnext.h5')

"""
## EVALUATION
noise_level = 15
model = tf.keras.models.load_model(f"unext.h5")
"""

psrn_list = []
for i in range(200, 450):
  patches_noisy,denoised_patches,ground_truth,noisy_image,denoised_image=predict_fun(model,f'data/test/{i}.png',noise_level=noise_level)
  print(i)
  ...
  path = f"result_convnext/noise_{noise_level}/{i}/"
  os.mkdir(path)
  ...
  plot_predictions(ground_truth, noisy_image, denoised_image, f"result_convnext/noise_{noise_level}/{i}.png")
